...
/**
 * Why a `vikunja` CLI run failed, as text for an error message.
 *
 * Vikunja reports failures through `log.Fatalf`, which writes a `level=ERROR`
 * line to **stdout** and exits 1, leaving stderr empty — so an error built from
 * stderr alone arrives with no reason at all ("could not reset the password: ").
 * Read both streams, keep the ERROR lines when there are any, and fall back to
 * the whole output, then to the exit code.
 */
